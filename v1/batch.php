...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($decoded !== null) {
        ...
        for ($i = 0; $i < count($decoded); $i++) {
            $obj = $decoded[$i];
            if ($obj->type == "Avatar") {
                $imageurl = "https://localhost" . getUserThumbnail($con, $obj->targetId);
            } elseif ($obj->type == "AvatarHeadShot") {
                $imageurl = "https://localhost" . getUserHeadshotThumbnail($con, $obj->targetId);
            } elseif ($obj->type == "Asset") {
                $userAgent = $_SERVER['HTTP_USER_AGENT'];
                $isRoblox = stripos($userAgent, 'Roblox') !== false;
                $imageUrl = "https://localhost" . getModelThumbnail($con, $obj->targetId, $isRoblox);
            } elseif ($obj->type == "PlaceIcon") {
                $imageurl = "https://localhost" . getPlaceIcon($con, $obj->targetId);
