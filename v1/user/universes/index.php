...
?>
<<<<<<< HEAD
{"previousPageCursor":null,"nextPageCursor":"<?php echo NoXSSPlz($cursor); ?>","data":[<?php while ($games = $gamequery->fetch()) {
      $count = $count + 1; ?>{"id":<?= $games['id']; ?>,"name":"<?php echo NoXSSPlz($games['name']); ?>","description":"<?php echo trim(preg_replace('/\s+/', ' ', $games['description'])); ?>","isArchived":false,"rootPlaceId":<?= $games['id']; ?>,"isActive":true,"privacyType":"Public","creatorType":"User","creatorTargetId":<?= $games['creatorid']; ?>,"creatorName":"<?php echo NoXSSPlz($username); ?>","created":"2022-08-18T09:18:15.47Z","updated":"2022-08-18T09:18:19.43Z"}<?php if ($count != $countgames) {
                      echo ",";
                  }
  } ?>]}
=======
