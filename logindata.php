...
//$teamupdate->execute(['id' => $placeId]);
    ...
    if (strpos($url, 'account-info') !== false) {
        ...
        if ($MembType == "None") {
            $MembValue = 0;
        } elseif ($MembType == "BuildersClub") {
            $MembValue = 1;
        } elseif ($MembType == "TurboBuildersClub") {
            $MembValue = 2;
        } elseif ($MembType == "OutrageousBuildersClub") {
            $MembValue = 3;
        } elseif ($MembType == "Premium") {
            $MembValue = 4;
        }
        ?>
    ...
    if (strpos($url, '/users/') !== false && strpos($url, '/friends') !== false) {
        ...
        if (strpos($headers['User-Agent'], "Android") !== false || strpos($headers['User-Agent'], "iPhone") !== false) {
            header("Location: https://localhost/users/friends");
            exit();
        }
        $friendDataArray = [];
...
<<<<<<< HEAD
        ...
        while ($friends = $isFriendQuery->fetch()) {
            ...
            $friendDataArray[] = [
                ...
                "AvatarUri" => "https://localhost" . getUserHeadshotThumbnail($con, $friendId),
                "AvatarFinal" => true,
